...
cargo run -p ninmu-mock-anthropic-service -- --bind 127.0.0.1:0
```
...
- `crates/ninmu-mock-anthropic-service/` — reusable mock Anthropic-compatible service
...
| Config file hierarchy (`.ninmu.json` + merged config sections) | ✅ |
| Permission system | ✅ |
...
├── Cargo.lock
└── crates/
    ├── ninmu-api/          # Provider clients + streaming + request preflight
    ├── ninmu-commands/     # Shared slash-command registry + help rendering
    ├── ninmu-compat-harness/ # TS manifest extraction harness
    ├── ninmu-mock-anthropic-service/ # Deterministic local Anthropic-compatible mock
    ├── ninmu-plugins/      # Plugin metadata, manager, install/enable/disable surfaces
    ├── ninmu-runtime/      # Session, config, permissions, MCP, prompts, auth/runtime loop
    ├── ninmu-cli/          # Main CLI binary (`ninmu`)
    ├── ninmu-telemetry/    # Session tracing and usage telemetry types
    └── ninmu-tools/        # Built-in tools, skill resolution, tool search, agent runtime surfaces
```

### Crate Responsibilities

- **ninmu-api** — provider clients, SSE streaming, request/response types, auth (`ANTHROPIC_API_KEY` + bearer-token support), request-size/context-window preflight
- **ninmu-commands** — slash command definitions, parsing, help text generation, JSON/text command rendering
- **ninmu-compat-harness** — extracts tool/prompt manifests from upstream TS source
- **ninmu-mock-anthropic-service** — deterministic `/v1/messages` mock for CLI parity tests and local harness runs
- **ninmu-plugins** — plugin metadata, install/enable/disable/update flows, plugin tool definitions, hook integration surfaces
- **ninmu-runtime** — `ConversationRuntime`, config loading, session persistence, permission policy, MCP client lifecycle, system prompt assembly, usage tracking
- **ninmu-cli** — REPL, one-shot prompt, direct CLI subcommands, streaming display, tool call rendering, CLI argument parsing
- **ninmu-telemetry** — session trace events and supporting telemetry payloads
- **ninmu-tools** — tool specs + execution: Bash, ReadFile, WriteFile, EditFile, GlobSearch, GrepSearch, WebSearch, WebFetch, Agent, TodoWrite, NotebookEdit, Skill, ToolSearch, and runtime-facing tool discovery
