...
fn detect_repo(cwd: &Path) -> RepoDetection {
    let package_json_contents = fs::read_to_string(cwd.join("package.json"))
        ...
        .to_ascii_lowercase();
    RepoDetection {
        rust_workspace: cwd.join("src").join("Cargo.toml").is_file(),
...
fn verification_lines(cwd: &Path, detection: &RepoDetection) -> Vec<String> {
    ...
    if detection.rust_workspace {
        lines.push("- Run Rust verification from `src/`: `cargo fmt`, `cargo clippy --workspace --all-targets -- -D warnings`, `cargo test --workspace`".to_string());
    ...
    if detection.rust_workspace {
        lines.push("- `src/` is the Rust workspace root; keep docs, workflows, and helper scripts aligned with layout changes.".to_string());
    }
    lines
}
...
mod tests {
    ...
    fn temp_dir() -> std::path::PathBuf {
        ...
        std::env::temp_dir().join(format!("ninmu-init-{nanos}-{counter}"))
    }
    ...
    fn initialize_repo_creates_expected_files_and_gitignore_entries() {
        ...
        fs::write(root.join("src").join("Cargo.toml"), "[workspace]\n").expect("write cargo");
