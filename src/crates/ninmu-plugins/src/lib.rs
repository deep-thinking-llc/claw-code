...
impl PluginManager {
    ...
    pub fn bundled_root() -> PathBuf {
        ...
        let mut candidates = vec![manifest_dir.join("bundled")];
        if let Ok(cwd) = std::env::current_dir() {
            candidates.push(cwd.join("crates").join("ninmu-plugins").join("bundled"));
            candidates.push(
                cwd.join("src")
                    .join("crates")
                    .join("ninmu-plugins")
        ...
        candidates
            .into_iter()
            .find(|candidate| {
                discover_plugin_dirs(candidate)
                    .map(|dirs| !dirs.is_empty())
                    .unwrap_or(false)
            })
            .unwrap_or_else(|| manifest_dir.join("bundled"))
    }
