...
/// Write current worker state to `.ninmu/worker-state.json` under the worker's cwd.
...
fn emit_state_file(worker: &Worker) {
    let state_dir = std::path::Path::new(&worker.cwd).join(".ninmu");
...
mod tests {
    ...
    fn emit_state_file_writes_worker_status_on_transition() {
        ...
        let state_path = cwd_path.join(".ninmu").join("worker-state.json");
        assert!(
            state_path.exists(),
