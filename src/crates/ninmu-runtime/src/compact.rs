...
mod tests {
    ...
    fn keeps_previous_compacted_context_when_compacting_again() {
        ...
        initial_session.messages = vec![
            ConversationMessage::user_text("Investigate src/crates/ninmu-runtime/src/compact.rs"),
            ConversationMessage::assistant(vec![ContentBlock::Text {
                text: "I will inspect the compact flow.".to_string(),
            }]),
            ConversationMessage::user_text(
                "Also update src/crates/ninmu-runtime/src/conversation.rs",
            ),
            ConversationMessage::assistant(vec![ContentBlock::Text {
        ...
        assert!(second
            .formatted_summary
            .contains("Also update src/crates/ninmu-runtime/src/conversation.rs"));
        assert!(matches!(
    ...
    fn extracts_key_files_from_message_content() {
        let files = collect_key_files(&[ConversationMessage::user_text(
            "Update src/crates/ninmu-runtime/src/compact.rs and src/crates/ninmu-cli/src/main.rs next.",
        )]);
        assert!(files.contains(&"src/crates/ninmu-runtime/src/compact.rs".to_string()));
        assert!(files.contains(&"src/crates/ninmu-cli/src/main.rs".to_string()));
    }
