...
fn build_count_tokens_response(request: &MockRequest, scenario: Scenario) -> String {
...
fn count_request_tokens(request: &MockRequest) -> u32 {
    let message_chars: usize = request
        .messages
        .iter()
        .flat_map(|message| &message.content)
        .map(|block| match block {
            MockContentBlock::Text(text) => text.len(),
            MockContentBlock::ToolUse { id, name } => id.len() + name.len(),
            MockContentBlock::ToolResult {
                tool_use_id,
                content,
                ..
    ...
    let rough_tokens = message_chars.div_ceil(4);
